#Defines CNN models for processing CFD simulation output and predict forces and reconstruct input
import numpy
...
def ccLayer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=True, useType="test", resuse_batch_norm=False, normalize=True, wd=0.0001):
	...
	conv_in, angles = cc.layers.conv(inpt, convW, convb, c_i, c_o, s_h, s_w, first=first, useType=useType, padding=padding, num_bins=16, normalize=normalize, count=False, weightMask=None)
...
def plainLayer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=True, useType="test", resuse_batch_norm=False, normalize=True, wd=1e-4, convW=None, convb=None,\
 activationFunction="LReLU", reguralizationMode=cc.misc.default_regularization_mode):
	if convW == None:
		convW, convb = cc.misc.getWeightsNBiases(first, k_h, k_w, c_o, c_i, s_h, s_w, wd=None, lossType=cc.misc.default_regularization, mode=reguralizationMode)
	...
	if normalize:
		out = cc.layers.batch_norm(conv_in, useType=useType, reuse=resuse_batch_norm)
	else:
		out = conv_in
...
def scalarModSpecificConv(inpt, modality, first=True, useType="test", varName="PKNut", resuse_batch_norm=False, fs=5, reguralizationMode=cc.misc.default_regularization_mode):
	#conv1
	with tf.variable_scope(varName+"Conv1", reuse=(not first)) as scope:
		k_h = fs; k_w = fs; c_o = 6; c_i = 1; s_h = 1; s_w = 1
		conv1W, conv1b = cc.misc.getWeightsNBiases(first, k_h, k_w, c_o, c_i, s_h, s_w, mode=reguralizationMode)
	...
	with tf.variable_scope(varName+"Conv2", reuse=(not first)) as scope:
		...
		conv2W, conv2b = cc.misc.getWeightsNBiases(first, k_h, k_w, c_o, c_i, s_h, s_w, mode=reguralizationMode)
	...
	with tf.variable_scope(varName+"Conv3", reuse=(not first)) as scope:
		...
		conv3W, conv3b = cc.misc.getWeightsNBiases(first, k_h, k_w, c_o, c_i, s_h, s_w, mode=reguralizationMode)
	...
	with tf.variable_scope(varName+"Conv4", reuse=(not first)) as scope:
		...
		conv4W, conv4b = cc.misc.getWeightsNBiases(first, k_h, k_w, c_o, c_i, s_h, s_w, mode=reguralizationMode)
...
def velocityCoherent(inpt, first=True, useType="test", resuse_batch_norm=False, fs=5, reguralizationMode=cc.misc.default_regularization_mode):
	#conv1
	with tf.variable_scope("vcConv1", reuse=(not first)) as scope:
		...
		conv1 = plainLayer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=True,\
		 wd=0.0001, reguralizationMode=reguralizationMode)
	#conv2
...
def cliffordConvolution(inpt, first=True, useType="test", resuse_batch_norm=False, fs=5):
	...
	with tf.variable_scope("ccConv1", reuse=(not first)) as scope:
		...
		conv1_relu, angles_1 = ccLayer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=0.001)
	...
	with tf.variable_scope("ccConv2", reuse=(not first)) as scope:
		...
		conv2_relu, angles_2 = ccLayer(conv1, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=0.001)
	...
	with tf.variable_scope("pool") as scope:
		...
		pooled = tf.nn.avg_pool(conv2+conv1, [1,2,2,1], [1,2,2,1], padding='VALID')
		pooled = cc.layers.normalizeVectorField(pooled, fs, fs)

	#conv3
	with tf.variable_scope("ccConv3", reuse=(not first)) as scope:
		k_h = fs; k_w = fs; c_o = 8; c_i = 4; s_h = 1; s_w = 1
		conv3_relu, angles_3 = ccLayer(pooled, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=0.001)
		conv3 = cc.transformations.changeToCartesian(conv3_relu, angles_3, False)
		conv3 = cc.layers.normalizeVectorField(conv3, fs, fs)
	#conv4
	with tf.variable_scope("ccConv4", reuse=(not first)) as scope:
		k_h = fs; k_w = fs; c_o = 8; c_i = 8; s_h = 1; s_w = 1
		conv4_relu, angles_4 = ccLayer(conv3, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=0.001)
	...
	with tf.variable_scope("pool2") as scope:
		...
		pooled2 = tf.nn.avg_pool(conv4+conv3, [1,2,2,1], [1,2,2,1], padding='VALID')
		# pooled2 = cc.layers.normalizeVectorField(pooled2, fs, fs)
	return pooled2
...
def inference(examples, first, useType="test", modelType="vc", fs=3, reguralizationMode=cc.misc.default_regularization_mode):
	with tf.variable_scope("Encoder") as scope:
		with tf.variable_scope("ScalarConvolutions") as scope:
			#P specific layers
			convP = scalarModSpecificConv(examples[1], "p", first, useType, resuse_batch_norm=not first, reguralizationMode=reguralizationMode)
			#nut specific layers
			convNut = scalarModSpecificConv(examples[2], "nut", False, useType, resuse_batch_norm=not first, reguralizationMode=reguralizationMode)
			#nuTilda specific layers
			convNuTilda = scalarModSpecificConv(examples[3], "nuTilda", False, useType, resuse_batch_norm=not first, reguralizationMode=reguralizationMode)

		with tf.variable_scope("velocityConvolutions") as scope:
			#U specific layers
			if modelType == "vc":
				convU = velocityCoherent(examples[0], first, useType, resuse_batch_norm=not first, reguralizationMode=reguralizationMode)
			elif modelType == "ds":
...
def predictForces(code, batch_size, log, useType="train", first=True, NUM_OUTPUTS=2, reguralizationMode=cc.misc.default_regularization_mode):
	with tf.variable_scope("MLPRegressor", reuse=(not first)) as scope:
		...
		with tf.variable_scope("fc1") as scope:
			...
			fc1W = cc.misc._variable_with_weight_decay('weights', shape=[dim, 512], stddev=0.01, wd=0.0001, mode=reguralizationMode)
		...
		with tf.variable_scope("fc2") as scope:
			...
			fc2W = cc.misc._variable_with_weight_decay('weights', shape=[512, 512], stddev=0.01, wd=0.0001, mode=reguralizationMode)
		...
		with tf.variable_scope("fc3") as scope:
			...
			fc3W = cc.misc._variable_with_weight_decay('weights', shape=[512, NUM_OUTPUTS], stddev=0.01, wd=0.0001, mode=reguralizationMode)
...
def predictFlow(code, batch_size, log, useType="train", first=True, NUM_OUTPUTS=2, reguralizationMode=cc.misc.default_regularization_mode):
	...
	with tf.variable_scope("flowPrediction", reuse=not first):	
		...
		with tf.variable_scope("Conv7") as scope:
			...
			conv7 = plainLayer(conv6, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=True, wd=0.0001)
...
def reconstruct(code, batch_size, log, useType="train", first=True, NUM_OUTPUTS=2, reguralizationMode=cc.misc.default_regularization_mode):
	resuse_batch_norm = not first
	with tf.variable_scope("flowReconstruction", reuse=not first):	
		#up1
		with tf.variable_scope("UpSample1") as scope:
			up1 = unPool2D(code, 2, 2)
			if up1.get_shape()[-2].value == 14:
				paddings = tf.constant([[0,0],[0,0],[0,1],[0,0]])
				up1 = tf.pad(up1, paddings)
		#conv1
		with tf.variable_scope("Conv1") as scope:
			k_h = 3; k_w = 3; c_o = 128; c_i = 128; s_h = 1; s_w = 1
			conv1 = plainLayer(up1, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=True, wd=0.0001)
